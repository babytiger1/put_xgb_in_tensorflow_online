...
DNN_L1 =CONF['dnn_l1']
DNN_L2 =CONF['dnn_l2']
OUTPUT_SIZE_XGB = CONF['output_size_xgb']
MAX_TREE_NODE = CONF['max_tree_node']
...
def _dnn_logit_fn(features, mode, model_id, units, hidden_units,
    ...
    with tf.variable_scope(
        ...
        conFeatureCol = feature_columns[0]
        FeatureCol_forxgb = feature_columns[1]
        catFeatureCol = feature_columns[2]
        coninputTensor = tf.feature_column.input_layer(features, conFeatureCol)
        xgbinputTensor = tf.feature_column.input_layer(features, FeatureCol_forxgb)
       # catinputTensor = tf.feature_column.input_layer(features, catFeatureCol)
        transform_XGB_Model = autograph.to_graph(testvars4.XGBprocess4)
        ouputType = [tf.int32] * OUTPUT_SIZE_XGB
        xgbFeature = tf.map_fn(transform_XGB_Model, xgbinputTensor, dtype=ouputType)
        xgbFeature = tf.concat([xgbFeature], axis=-1)
        xgbFeature = tf.transpose(xgbFeature, [1, 0])
        xgbFeature = tf.one_hot(xgbFeature)
        xgbFeature = tf.reshape(xgbFeature,[-1,OUTPUT_SIZE_XGB*MAX_TREE_NODE])
        input_layer = tf.concat([xgbFeature,coninputTensor],axis=-1)
        # word_embeddings = tf.get_variable("word_embeddings",[OUTPUT_SIZE_XGB*MAX_TREE_NODE, EMBEDDING_SIZE])
        # embedded_word_ids = tf.nn.embedding_lookup(word_embeddings, xgbFeature)

        net = input_layer
        if connected_mode == 'simple':
            for layer_id, num_hidden_units in enumerate(hidden_units):
                with tf.variable_scope(
                        'dnn_{}/hiddenlayer_{}'.format(model_id, layer_id),
        ...
        with tf.variable_scope(
                'dnn_{}/logits'.format(model_id), values=(net, )) as logits_scope:
...
def multidnn_logit_fn_builder(units, hidden_units_list, connected_mode_list,
                              feature_columns, input_layer_partitioner):
    ...
    if not isinstance(hidden_units_list[0], (list, tuple)):
        hidden_units_list = [hidden_units_list
                             ]  # compatible for single dnn input hidden_units
    ...
    def multidnn_logit_fn(features, mode):
        logits = []
        for idx, (hidden_units, connected_mode) in enumerate(
                zip(hidden_units_list, connected_mode_list)):
            logits.append(
                _dnn_logit_fn(features, mode, idx + 1, units, hidden_units,
                              connected_mode, feature_columns,
