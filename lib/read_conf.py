...
BASE_DIR = './conf'
SCHEMA_CONF_FILE = 'schema_criteo.yaml'       # feature name ，for example  1：D1
FEATURE_CONF_FILE = 'feature_criteo1.yaml'     # type transform （min-max ， file ，hash） distribution
MODEL_CONF_FILE = 'dnn_model.yaml'
...
class Config(object):
    ...
    def read_schema(self):
        with open(self._schema_conf_file) as f:
            return {k: v for k, v in yaml.load(f).items()}
    ...
    #     if type_ == 'category':
    #         assert trans in {
    #             'hash_bucket', 'identity', 'vocab'
    #         }, ("Invalid transform `{}` for feature `{}` in feature conf, "
    #             "must be one of `hash_bucket`, `vocab`, `identity`.".format(
    #                 trans, feature))
    #         if trans == 'hash_bucket' or trans == 'identity':
    #             if not isinstance(param, int):
    #                 raise TypeError(
    #                     'Invalid parameter `{}` for feature `{}` in feature conf, '
    #                     '{} parameter must be an integer.'.format(
    #                         param, feature, trans))
    #         elif trans == 'vocab':
    #             if not isinstance(param, (tuple, list)):
    #                 raise TypeError(
    #                     'Invalid parameter `{}` for feature `{}` in feature conf, '
    #                     'vocab parameter must be a list.'.format(
    #                         param, feature))
    #     else:
    #         normalization, boundaries = param['normalization'], param[
    #             'boundaries']
    #         if trans:
    ...
    #                 if not isinstance(
    #                         normalization,
    #                     (list, tuple)) or len(normalization) != 2:
    #                     raise TypeError(
    #                         'Invalid normalization parameter `{}` for feature `{}` in feature conf, '
    #                         'must be 2 elements list for `min_max` or `standard` scaler.'
    #                         .format(normalization, feature))
    #             if trans == 'min_max':
    #                 min_, max_ = normalization
    #                 if not isinstance(min_, (float, int)) or not isinstance(
    #                         max_, (float, int)):
    #                     raise TypeError(
    #                         'Invalid normalization parameter `{}` for feature `{}` in feature conf, '
    #                         'list elements must be int or float.'.format(
    #                             normalization, feature))
    #                 assert min_ < max_, (
    #                     'Invalid normalization parameter `{}` for feature `{}` in feature conf, '
    #                     '[min, max] list elements must be min<max'.format(
    #                         normalization, feature))
    #             elif trans == 'standard':
    ...
    #                     raise TypeError(
    #                         'Invalid normalization parameter `{}` for feature `{}` in feature conf, '
    #                         'parameter mean must be int or float.'.format(
    #                             mean, feature))
    #                 if not isinstance(std, (float, int)) or std <= 0:
    #                     raise TypeError(
    #                         'Invalid normalization parameter `{}` for feature `{}` in feature conf, '
    #                         'parameter std must be a positive number.'.format(
    #                             std, feature))
    #         if boundaries:
    #             if not isinstance(boundaries, (tuple, list)):
    #                 raise TypeError(
    #                     'Invalid parameter `{}` for feature `{}` in feature conf, '
    #                     'discretize parameter must be a list.'.format(
    #                         boundaries, feature))
    #             else:
    #                 for v in boundaries:
    ...
    def _read_model_conf(self):
        ...
        req_str_keys = [
            'dnn_optimizer', 'dnn_connected_mode','dnn_activation_function'
        ]
        # optional int or float params
        opt_num_keys = [
            'dnn_initial_learning_rate', 'dnn_decay_rate', 'dnn_l1', 'dnn_l2','output_size_xgb','dnn_dropout',
            "embedding_size"
        ]
    ...
    def _read_train_conf(self):
        ...
        req_num_keys = [
            'train_epochs', 'epochs_per_eval', 'batch_size', 'num_examples'
        ]
        opt_num_keys = [
            'pos_sample_loss_weight', 'neg_sample_loss_weight',
            'num_parallel_calls'
        ]
    ...
    @staticmethod   #check for numeric parameter
    def _check_numeric(key, value):
        if not isinstance(value, (int, float)):
            raise ValueError(
                'Numeric type is required for key `{}`, found `{}`.'.format(
                    key, value))  # che

    @staticmethod  #check for string parameter
    def _check_string(key, value):
        if not isinstance(value, (str, str )):
            raise ValueError(
                'String type is required for key `{}`, found `{}`.'.format(
                    key, value))

    @staticmethod  #check for bool parameter
    def _check_bool(key, value):
        if value not in {True, False, 1, 0}:
            raise ValueError(
                'Bool type is required for key `{}`, found `{}`.'.format(
                    key, value))

    @staticmethod  #check for list parameter
    def _check_list(key, value):
        if not isinstance(value, (list, tuple)):
            raise ValueError(
                'List type is required for key `{}`, found `{}`.'.format(
                    key, value))

    @staticmethod  #check for requier parameter
    def _check_required(key, value):
        if value is None:
            raise ValueError(
                'Required type for key `{}`, found None.'.format(key))
    ...
    def get_feature_name(self, feature_type='all'):
        ...
        if feature_type == 'all':
            ...
            feature_used = list(feature_conf_dic.keys())
            feature_used.sort()
            return feature_used
            ...
            return [
                feature for feature, des in feature_conf_dic.items()
                if des.stipe() == ''
            ]
            ...
            raise ValueError(
                "Invalid parameter, must be one of 'all', 'used', 'category, 'continuous"
            )


def _testz():
    config = Config()
    """test.txt for Config methods"""
    print('\nTrain config:')
    print(config.config)
    print(config.train)
